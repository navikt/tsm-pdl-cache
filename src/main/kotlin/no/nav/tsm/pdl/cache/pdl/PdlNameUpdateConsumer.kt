...
@Service
class PdlNameUpdateConsumer(val pdlPersonService: PdlPersonService, val personService: PersonService) {

    private val logger = LoggerFactory.getLogger(PdlNameUpdateConsumer::class.java)

    @WithSpan
    @KafkaListener(id = "tsm-pdl-cache-consumer-name-udpate-3", topics = ["pdl.pdl-persondokument-v1"])
    fun consume(record: ConsumerRecord<String, String?>) {
        ...
        val person = personService.getPerson(record.key())

        if(person.identer.firstOrNull { !it.historisk && it.gruppe == IDENT_GRUPPE.AKTORID && it.ident == record.key()} == null) {
            logger.warn("person with aktorId ${record.key()} is not currently active")
            return
        }
